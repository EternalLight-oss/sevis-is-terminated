  ...
  try {
    ...
    data.forEach((item) => {
      if (item.academic_level) {
        levelCounts[item.academic_level] =
          (levelCounts[item.academic_level] || 0) + 1;
      }
    });

    // Convert to array of objects for charting
    const academicLevelData = Object.entries(levelCounts).map(
      ([name, count]) => ({
        name,
        count,
      })
...
export async function getFingerprintingData() {
  try {
    ...
    // Convert to array of objects for charting
    const fingerprintingData = Object.entries(responseCounts).map(
      ([name, count]) => ({
        name,
        count,
      })
...
export async function getH1BStatusData() {
  try {
    ...
    data.forEach((item) => {
      if (item.h1b_status) {
        statusCounts[item.h1b_status] =
          (statusCounts[item.h1b_status] || 0) + 1;
      }
    });
  ...
  try {
    ...
    data.forEach((item) => {
      if (item.legal_case_status) {
        statusCounts[item.legal_case_status] =
          (statusCounts[item.legal_case_status] || 0) + 1;
      }
    });

    // Convert to array of objects for charting
    const legalCaseStatusData = Object.entries(statusCounts).map(
      ([name, count]) => ({
        name,
        count,
      })
...
export async function getIncidentTypeData() {
  try {
    ...
    // Convert to array of objects for charting
    const incidentTypeData = Object.entries(typeCounts).map(
      ([name, count]) => ({
        name,
        count,
      })
  ...
  try {
    ...
    data.forEach((item) => {
      if (item.sevis_notification_method) {
        methodCounts[item.sevis_notification_method] =
          (methodCounts[item.sevis_notification_method] || 0) + 1;
      }
    });

    // Convert to array of objects for charting
    const notificationMethodData = Object.entries(methodCounts).map(
      ([name, count]) => ({
        name,
        count,
      })
...
export async function getNationalityDistributionData() {
  try {
    const supabase = createServerSupabaseClient();
    ...
    if (error) {
      console.error("Error getting nationality distribution data:", error);
      throw new Error("Failed to get nationality distribution data");
    }
    ...
    data.forEach((item) => {
      if (item.nationality) {
        nationalityCounts[item.nationality] =
          (nationalityCounts[item.nationality] || 0) + 1;
      }
    });

    // Convert to array of objects for charting
    const nationalityDistribution = Object.entries(nationalityCounts).map(
      ([name, count]) => ({
        name,
        count,
      })
    ...
    console.error("Error in getNationalityDistributionData:", error);
    return []; // Return empty array if there's an error
  }
}
